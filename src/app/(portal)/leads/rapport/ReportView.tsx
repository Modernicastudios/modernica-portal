  ...
  return (
    <div style={{ maxWidth: 900, margin: '0 auto', padding: '20px 24px', background: '#FCFBFF', minHeight: '100vh' }}>
      ...
      <div className="no-print" style={{ marginBottom: 24 }}>
        <Link href="/leads" style={{ display: 'inline-flex', alignItems: 'center', gap: 6, color: 'var(--muted)', fontSize: 13, textDecoration: 'none', marginBottom: 12 }}>
          <ArrowLeft size={14} /> Terug naar leads
        </Link>

        <div style={{ background: 'white', border: '1px solid var(--border)', borderRadius: 12, padding: 16, marginBottom: 20 }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--accent1)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 10 }}>Periode kiezen</div>
          ...
          <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
            <span style={{ fontSize: 12, color: 'var(--muted)' }}>Of custom:</span>
            ...
            <button onClick={applyCustom} style={{
              padding: '8px 14px', background: 'var(--accent1)', color: 'white', border: 'none',
      ...
      <div id="report">
        {/* Header */}
        <div style={{ marginBottom: 32, paddingBottom: 20, borderBottom: '2px solid var(--accent1)' }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--accent1)', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 4 }}>Modernica CRM Rapport</div>
        ...
        <section style={{ marginBottom: 32 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(180px,1fr))', gap: 12 }}>
            <Kpi label="Gesprekken gelogd" value={calls.length} color="var(--accent1)" />
          ...
          <section style={{ marginBottom: 32 }}>
            ...
            <div style={{ background: 'white', border: '1px solid var(--border)', borderRadius: 12, padding: 16 }}>
              {Object.entries(callsByUser).sort((a, b) => b[1].count - a[1].count).map(([id, { name, count }]) => (
                <div key={id} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid var(--bg)' }}>
                  <span>{name}</span>
        ...
        {meetings.length > 0 && (
          <section style={{ marginBottom: 32 }}>
            ...
            <div style={{ background: 'white', border: '1px solid var(--border)', borderRadius: 12 }}>
              {meetings.map((m, i) => (
                <div key={m.id} style={{ padding: 14, borderBottom: i === meetings.length - 1 ? 'none' : '1px solid var(--bg)' }}>
                  ...
                  {m.notes && <div style={{ fontSize: 13, color: 'var(--muted)' }}>{m.notes}</div>}
                </div>
              ))}
            </div>
          </section>
        ...
        {calls.length > 0 && (
          <section style={{ marginBottom: 32 }}>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12, background: 'white', border: '1px solid var(--border)', borderRadius: 12, overflow: 'hidden' }}>
              <thead>
              ...
              <tbody>
                {calls.map((c: any) => {
                  ...
                  return (
                    <tr key={c.id} style={{ borderTop: '1px solid var(--bg)' }}>
        ...
        {/* Footer */}
        <div style={{ marginTop: 40, paddingTop: 16, borderTop: '1px solid var(--border)', fontSize: 11, color: '#8F8AA3', textAlign: 'center' }}>
...
function PresetBtn({ active, label, onClick }: any) {
  return (
    <button onClick={onClick} style={{
      ...
      background: active ? 'var(--accent1)' : 'white',
...
function Kpi({ label, value, color }: any) {
  return (
    <div style={{ padding: 16, background: 'white', border: '1px solid var(--border)', borderRadius: 12 }}>
...
const th: React.CSSProperties = { padding: '10px 12px', textAlign: 'left', fontSize: 11, fontWeight: 700, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '0.06em' }
