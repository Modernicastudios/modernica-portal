...
interface Props {
  ...
  pendingPosts: { id: string; client_id?: string | null }[]
  todayPosts: any[]
  upcomingPosts: any[]
  activeProjects: any[]
  openTasks: any[]
...
export default function DashboardClient({
  ...
  // Filter lists by selected client (for admins using the global client filter)
  const filteredPendingCount = useMemo(() => {
    if (!selectedClientId) return pendingPosts.length
    return pendingPosts.filter(p => p.client_id === selectedClientId).length
  }, [pendingPosts, selectedClientId])

  const filteredTodayPosts = useMemo(() => {
    if (!selectedClientId) return todayPosts
    return todayPosts.filter(p => p.client_id === selectedClientId)
  }, [todayPosts, selectedClientId])

  const filteredUpcomingPosts = useMemo(() => {
    if (!selectedClientId) return upcomingPosts
    return upcomingPosts.filter(p => p.client_id === selectedClientId)
  }, [upcomingPosts, selectedClientId])
  ...
  const filteredOpenTasks = useMemo(() => {
    if (!selectedClientId) return openTasks
    return openTasks.filter(t => t.projects?.client_id === selectedClientId)
  }, [openTasks, selectedClientId])

  const filteredRoiEntries = useMemo(() => {
    if (!selectedClientId) return roiEntries
    return roiEntries.filter(e => e.client_id === selectedClientId)
  }, [roiEntries, selectedClientId])

  const filteredPlatformPosts = useMemo(() => {
    if (!selectedClientId) return platformPostsRaw
    return platformPostsRaw.filter(p => p.client_id === selectedClientId)
  }, [platformPostsRaw, selectedClientId])

  // ROI aggregations
  const thisMonthRoi = filteredRoiEntries.filter((e: any) => e.month === currentMonth && e.year === currentYear)
  ...
  for (const post of filteredPlatformPosts) {
        ...
        <Link href="/approve" style={{ textDecoration: 'none', display: 'block', marginBottom: '20px' }}>
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              ...
              }}>{filteredPendingCount}</span>
              <div>
                <div style={{ fontWeight: 700, fontSize: '.9rem', color: 'var(--text)' }}>
                  {filteredPendingCount === 1 ? '1 post wacht op goedkeuring' : `${filteredPendingCount} posts wachten op goedkeuring`}
                </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: '20px', alignItems: 'start' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          ...
          <div style={{
            ...
            {/* Posts vandaag */}
            {filteredTodayPosts && filteredTodayPosts.length > 0 ? (
              filteredTodayPosts.map((post: any) => {
          ...
          {/* DEZE WEEK */}
          {filteredUpcomingPosts && filteredUpcomingPosts.length > 0 && (
            <div style={{
              background: 'var(--card)', border: '1px solid var(--border)',
              borderRadius: 'var(--radius)', overflow: 'hidden',
            }}>
              ...
              {filteredUpcomingPosts.map((post: any) => {
                const platform = (post.platform || '').toLowerCase()
                const color = PLATFORM_COLORS[platform] || '#6b7280'
          ...
          {isAdmin && filteredRoiEntries.length > 0 && (
            <div style={{
              background: 'var(--card)', border: '1px solid var(--border)',
              borderRadius: 'var(--radius)', overflow: 'hidden',
            }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '0' }}>
                {[
                  { label: 'Totaal spend',  value: fmtEur(roiSpend),   color: '#1877f2' },
                  { label: 'Totaal omzet',  value: fmtEur(roiRevenue), color: '#00b89c' },
                  { label: 'Gem. ROAS',     value: roiSpend > 0 ? `${roiRoas.toFixed(2)}×` : '—', color: roasColor },
                  { label: 'Leads',         value: roiLeads.toString(), color: 'var(--accent1)' },
                ].map((kpi, i, arr) => (
                  <Link key={kpi.label} href="/roi" style={{ textDecoration: 'none' }}>
                    <div style={{
                      ...
                      <div style={{ fontWeight: 800, fontSize: '1.35rem', color: kpi.color, fontFamily: 'var(--font-syne), sans-serif' }}>
                        {kpi.value}
                      </div>
                    </div>
                  </Link>
                    ...
                    return (
                      <Link key={platform} href="/content" style={{ textDecoration: 'none' }}>
                        <div style={{
                          ...
                          <span style={{ fontSize: '.82rem', fontWeight: 600, color }}>{label}</span>
                          <span style={{ background: color, color: '#fff', borderRadius: '50px', padding: '1px 7px', fontSize: '.7rem', fontWeight: 800 }}>
                            {count}
                          </span>
                        </div>
                      </Link>
          ...
          {filteredActiveProjects && filteredActiveProjects.length > 0 && (
            <div style={{
              ...
              {filteredActiveProjects.map((project: any) => {
                ...
                return (
                  <div key={project.id} style={{
                    ...
                    <span style={{ fontSize: '.7rem', fontWeight: 700, padding: '3px 9px', borderRadius: '50px', background: sc.bg, color: sc.color, flexShrink: 0 }}>
                      {sc.label}
                    </span>
                  </div>
                )
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          <div style={{
            ...
            {[
              { label: 'Wachten op goedkeuring', value: filteredPendingCount, color: 'var(--accent1)', href: '/approve' },
              { label: 'Posts vandaag', value: filteredTodayPosts?.length ?? 0, color: '#e1306c', href: '/content' },
              { label: 'Open taken', value: filteredOpenTasks?.length ?? 0, color: '#f5a623', href: '/projects' },
            ].map((stat, i, arr) => (
          ...
          {isAdmin && thisMonthRoi.length > 0 && (
            <Link href="/roi" style={{ textDecoration: 'none' }}>
              <div style={{
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: '.72rem', color: 'var(--muted)', marginBottom: '3px' }}>ROI deze maand</div>
          ...
          {/* OPEN TAKEN */}
          {filteredOpenTasks && filteredOpenTasks.length > 0 && (
            <div style={{
              background: 'var(--card)', border: '1px solid var(--border)',
              borderRadius: 'var(--radius)', overflow: 'hidden',
            }}>
              ...
              {filteredOpenTasks.map((task: any) => (
                <div key={task.id} style={{
                  padding: '11px 18px', borderBottom: '1px solid var(--border)',
                  display: 'flex', alignItems: 'flex-start', gap: '10px',
                }}>
                  <div style={{ width: '16px', height: '16px', borderRadius: '50%', border: '2px solid var(--border)', flexShrink: 0, marginTop: '2px' }} />
              ...
              <Link href="/projects" style={{ display: 'block', padding: '11px 18px', fontSize: '.78rem', color: 'var(--accent1)', textDecoration: 'none', fontWeight: 600 }}>
                Alle taken bekijken →
              </Link>
            </div>
          )}
